...
export function registerRun(program: Command): void {
  program
    ...
    .option("--generation-output-tokens <number>", "Per-generation model output cap (default 4096)")
    ...
    .option("--finalizer-output-tokens <number>", "Per-finalizer model output cap (default 4096)")
    .option("--max-critique-chars <number>", "Maximum deduplicated aggregate critique characters (default 16000)")
    .option("--provider-max-concurrency <number>", "Maximum process-shared concurrency for this provider route")
    .option("--rank-mode <mode>", "Final ranking schedule: all-pairs|k-regular")
    .option("--rank-k <number>", "Degree for k-regular final ranking")
    .option("--rank-seed <number>", "Seed for final-ranking pair scheduling")
    .option("--rank-max-calls <number>", "Logical judge-call cap for the final ranking schedule")
